...
export default function DS(){
  ...
  useEffect(()=>{
    ...
    function completeQuest(){p.questPoints=(p.questPoints||0)+1;p.rep.guard=(p.rep.guard||0)+1;completeQuest();}
      ...
      if(p.act==="combat"&&p.cmb){
        ...
        if(p.at>=p.as&&dist(p,mon)<=attackRange){p.at=0;
          ...
          if(cw?.rng){
            ...
            const hitC=p.eagleEye>0?1:(0.32+(rl+rngBuff)*0.016);
            ...
            if(Math.random()<hitC){
              let maxH=Math.max(1,Math.floor((rl+rngBuff)*0.55))+1;let hit=Math.floor(Math.random()*maxH*rngWeak)+1;
  ...
  function saveGame(){if(!p||!gR.current)return;const g2=gR.current;try{localStorage.setItem("dunescape_save",JSON.stringify({ver:SAVE_VERSION,sk:p.sk,inv:p.inv,eq:p.eq,bank:p.bank,hp:p.hp,mhp:p.mhp,prayer:p.prayer,maxPrayer:p.maxPrayer,quests:p.quests,desertKills:p.desertKills,goblinKills:p.goblinKills||0,totalXp:p.totalXp,x:p.x,y:p.y,runE:p.runE,achievements:p.achievements,autoRetaliate:p.autoRetaliate,slayerTask:p.slayerTask,haunted:p.haunted,jogreKills:p.jogreKills,demonKills:p.demonKills,jadKills:p.jadKills,relicParts:p.relicParts,buffs:p.buffs,ironman:p.ironman,visitedRegions:[...(p.visitedRegions||[])],cookCount:p.cookCount,activePrayers:p.activePrayers||[],shipmentFish:p.shipmentFish||0,iceWarriorKills:p.iceWarriorKills||0,pet:p.pet,questPoints:p.questPoints||0,unlocks:p.unlocks||[],rep:p.rep||{guard:0,merchant:0,bandit:0},lastFireTile:p.lastFireTile,prestige:p.prestige||{},farmPatches:g2.objects?.filter(o=>o.t==="farm_patch").map(o=>({id:o.id,seed:o.seed,readyAt:o.readyAt,grown:o.grown})),playerName:p.playerName||"Adventurer",camp:p.camp,campBank:p.campBank||[],appearance:p.appearance||{skin:"#f0d8a0",hair:"#333",outfit:"#2266cc"},codex:p.codex||[],sideQuests:p.sideQuests||[],dailyChallengeProgress:p.dailyChallengeProgress||0}));addC("Game saved!");}catch(e){}}
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",minHeight:0}}>
        ...
        <div style={{width:195,background:"linear-gradient(180deg,#1e0a06,#180804)",borderLeft:"2px solid #5a1808",display:"flex",flexDirection:"column",flexShrink:0}}>
          ...
          <div style={{flex:1,overflow:"auto",padding:5}}>
            ...
            {tab==="bestiary"&&<div style={{padding:4}}>
              <div style={{color:"#c8a84e",fontSize:10,fontWeight:700,letterSpacing:1,marginBottom:6}}>BESTIARY</div>
              {genMons().map((m,i)=><div key={i} style={{background:"rgba(70,20,5,0.45)",padding:"5px 8px",borderRadius:4,marginBottom:3,border:"1px solid rgba(200,168,78,0.08)"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{fontSize:10,color:m.c,fontWeight:600}}>{m.nm} <span style={{color:"#888",fontSize:8}}>(Lvl {m.lvl})</span></div>
                  <div style={{fontSize:8,color:"#555"}}>❤️{m.mhp}</div>
                </div>
                <div style={{fontSize:8,color:"#888",marginTop:1,fontStyle:"italic",lineHeight:1.4}}>{m.examine}</div>
                <div style={{fontSize:7,color:"#c04020",marginTop:1}}>Weak: {m.weak||"none"}</div>
                <div style={{fontSize:7,color:"#666",marginTop:1}}>Drops: {m.drops.slice(0,4).map(d=>{const it=ITEMS[d.i];const rate=d.c>=1?"always":d.c>=0.1?"common":d.c>=0.02?"uncommon":"rare";return (it?.n||d.i)+" ("+rate+")";}).join(", ")}{m.drops.length>4&&", ..."}</div>
              </div>)}
            </div>}
